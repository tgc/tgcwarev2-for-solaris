...
global_config_args="--prefix=$lprefix --with-local-prefix=$prefix --bindir=${prefix}/bin --mandir=${prefix}/man --infodir=${prefix}/info --with-libiconv-prefix=$lprefix --with-gmp=$lprefix --with-mpfr=$lprefix --with-mpc=$lprefix --disable-nls --enable-shared --enable-threads=posix"
...
[ "$_os" = "sunos58" ] && platform_configure_args="--enable-obsolete"

platform_configure_args="$platform_configure_args $cpu"
...
setup_tools()
{
    ...
    # in an invalid sysinfo.go file being generated
...
install()
{
    ...
    ${__ln_s} ../../../libobjc.so.4 libobjc.so
    ${__ln_s} ../../../libssp.so.0 libssp.so
    ${__ln_s} ../../../libstdc++.so.6 libstdc++.so
    ${__ln_s} ../../../libquadmath.so.0 libquadmath.so
    ${__ln_s} ../../../libgo.so.0 libgo.so
    ${__ln_s} ../../../libitm.so.1 libitm.so
    # For Ada
    ...
    cd $FULLPATH/adalib
    ${__ln_s} ../../../../libgnarl-*.so libgnarl.so
    ${__ln_s} ../../../../libgnarl-*.so libgnarl-${majorminor}.so
    ${__ln_s} ../../../../libgnat-*.so libgnat.so
    ${__ln_s} ../../../../libgnat-*.so libgnat-${majorminor}.so
    cd ..
